...
This file lists the core backend API endpoints, their request/response formats, and the data sources they rely on.

Each endpoint is documented in OpenAPI as well (see `openapi.yaml`).
...
Example
/api/logs/search?service=auth-service&level=ERROR&limit=20
Response
